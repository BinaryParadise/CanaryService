...
extension MockDataViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.onSwitch = { [weak self] (mock, isOn) in
            URLRequest.post(with: "/api/mock/active", params: ["id": mock.id, "enabled": isOn]) { [weak self] (result, error) in
                if result.code == 0 {
                    mock.enabled = isOn
...
extension MockDataViewCell: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        guard let mock = mock else { return }
        if let scene = mock.scenes?[safe: indexPath.row] {
            URLRequest.post(with: "/api/mock/scene/active", params: ["sceneid": scene.id, "enabled": mock.enabled, "id": mock.id]) { [weak self] (result, error) in
                if result.code == 0 {
