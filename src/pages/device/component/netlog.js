...
export default class NetLogPage extends React.Component {
    ...
    render() {
        ...
        return (<Drawer
            width={800}
            height={600}
            title={<span><span style={{ color: 'red', marginRight: 6 }}>{data.statusCode}</span><span style={{ color: 'purple', marginRight: 6 }}>{data.method}</span><Link to={routerURL('/request', data)} target="_blank" style={{ color: 'orange' }}>{data.url}</Link></span>}
            placement="bottom"
            closable={true}
